import {
    SRC_DIR, SCRIPTS_EXTENSIONS,
    STYLE_EXTENSIONS, FILE_EXTENSION, STYLE_EXTENSION, scriptExtNames,
    IMPORT_FROM_DEPEDENCE_RE,
    EXPORT_FROM_DEPEDENCE_RE,
    IMPORT_DEPEDENCE_RE,
    CSS_RE,
    LESS_RE,
    SCSS_RE
...
function tryMatchExt(file: string, fileList: string[]) { //  查找文件后缀
...
export async function completeSuffixDep(file: string, code: string) { // 补全 开发环境的代码后缀
    ...
    const replacer = function (code: string, ext: string) {
        ...
        const done = (targetDep: string) => code.replace(depFile, targetDep)
        if (isNodeModule) {
            return code
        }
...
export async function compileScriptFile(file: string) {// 编译js文件
    let source = readFileSync(file, 'utf-8')
    await compileScript(source, file)
}

export async function compileScript(content: string, filePath: string) { //编译js文件

    // let {format} =await getKeyLionConfig()
    if (isJsx(filePath)) { // 是否为jsx
        const babelResul = await babel.transformAsync(content, { // 直接babel解析
            filename: filePath,
            babelrc: false,
        ...
        if (babelResul?.code) { // 拿到编译后的代码
            ({ code: content } = babelResul)
        }
    }
    let { code } = await esb.transform(content, { // 把ts部分编译掉
        loader: "ts",
        target: "es2016",
        format: "esm"
    })
