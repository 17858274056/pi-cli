...
export async function compilerDir(dirPath: string) { // 解析每个组件内的文件
    let fileContent = await readdir(dirPath)
    await Promise.all(fileContent.map((filename) => { // 逐个拼接解析
...
export async function compileFile(filename: string, path: string) { // 编译文件处理.vue .css .less .scss等
...
export async function compileMoudle() {
    ...
    await Promise.all(fileDir.map(file => {
        ...
        if (!ignoreFile.includes(file)) ensureFileSync(resolve(curFile, './style/index.mjs')); // 查找有没有统一的style
